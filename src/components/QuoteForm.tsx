...
const SECTION_COLORS: Record<Section, string> = {
  ...
  Downstairs: 'bg-slate-50 text-slate-700 border-slate-200',
}
                  ...
                  {SECTIONS.map(section => {
                    ...
                    return (
                      <div key={section} className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${isUp ? '#DDD6FE' : '#c7d2fe'}` }}>
                        {/* Section header */}
                        <div className={`px-4 pt-3 pb-2 ${isUp ? 'bg-teal-50' : 'bg-slate-50'}`}>
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs font-bold uppercase tracking-widest ${isUp ? 'text-teal-700' : 'text-slate-700'}`}>
                                {section}
                              </span>
                              <span className={`text-xs ${isUp ? 'text-teal-500' : 'text-slate-400'}`}>
                                {sectionRooms.length} room{sectionRooms.length !== 1 ? 's' : ''}
                              </span>
                            </div>
                            {sectionTotal > 0 && (
                              <span className={`text-xs font-bold ${isUp ? 'text-teal-700' : 'text-slate-700'}`}>
                          ...
                          <select
                            ...
                            className={`w-full text-sm font-semibold rounded-xl px-3 py-2 border focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white ${isUp ? 'border-teal-200 text-teal-800' : 'border-teal-200 text-teal-800'}`}
                          >
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-2">
                            {(['material', 'labor'] as const).map(kind => (
                              <div key={kind} className="flex items-center rounded-xl border bg-white overflow-hidden focus-within:ring-2 focus-within:ring-teal-500 focus-within:border-teal-400" style={{ borderColor: isUp ? '#DDD6FE' : '#c7d2fe' }}>
                                <span className={`px-2.5 py-2.5 text-xs font-bold border-r ${isUp ? 'text-teal-600 border-teal-100' : 'text-slate-600 border-slate-100'}`} style={{ background: isUp ? '#F0FDFA' : '#F8FAFC' }}>$</span>
                                <input
                                  type="number"
                                  inputMode="decimal"
                                ...
                                />
                                <span className={`px-2 py-2.5 text-[10px] font-bold border-l ${isUp ? 'text-teal-500 border-teal-100' : 'text-slate-500 border-slate-100'}`} style={{ background: isUp ? '#F0FDFA' : '#F8FAFC' }}>
                        ...
                        <div className="p-3 space-y-2 bg-white">
                          ...
                          <button
                            ...
                            className={`w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold transition-colors ${isUp ? 'text-teal-600 hover:bg-teal-50' : 'text-slate-600 hover:bg-slate-50'}`}
                          >
        ...
        <div className="hidden lg:block lg:col-span-1">
          <div className="sticky top-4 space-y-4">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, var(--primary) 0%, #0B7D73 100%)', boxShadow: '0 4px 14px rgba(13,148,136,0.4)' }}
            >
